###################################################################
### CHECKING FOR SPATIAL AUTOCORRELATION IN THE MERGED NETWORK DATA
### USING dbMem and forward seletion
...
### coordinates
xyTiw <- subset(dat1, Site == "Tiwoho",
                select=c(longitude_x, latitude_y))

### potentially interesting network indices
indicesTiw <- subset(dat1, Site == "Tiwoho",
                     select=c(weighted_connectance, weighted_NODF,
                              generality.HL, robustness.HL, 
                              interaction_evenness, vulnerability.LL,
                              H2, no._interactions, no._interactions_video, 
